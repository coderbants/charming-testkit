...
//!
//! <user-docs>
//! `PtySession` provides real pseudo-terminal interaction on Unix. On Windows
//! the same public type remains available so dependent crates compile, but
//! spawning and PTY operations return an explicit unsupported error until a
//! native Windows backend is implemented.
//! </user-docs>
...
mod unsupported {
    ...
    const UNSUPPORTED_MESSAGE: &str =
        "PtySession requires a Unix pseudo-terminal; native Windows PTY support is deferred";

    /// A compile-safe placeholder for the Unix PTY session on non-Unix hosts.
    ///
    /// The type preserves the public API so crates can compile and expose their
    /// Windows behavior without importing Unix file-descriptor, signal, fork,
    /// ioctl, or wait APIs. Spawning is intentionally unsupported until a
    /// native Windows PTY backend is implemented.
    #[derive(Debug)]
    pub struct PtySession;

    impl PtySession {
        /// Reports that PTY spawning is not supported on this host.
        pub fn spawn(_cmd: &str, _args: &[&str]) -> Result<PtySession> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }
        ...
        /// Returns an empty capture because no PTY session can be created.
        pub fn raw_output(&self) -> Vec<u8> {
            Vec::new()
        }

        /// Returns an empty screen because no PTY session can be created.
        pub fn screen(&self) -> ScreenState {
            ScreenState::replay(&[], DEFAULT_ROWS as usize, DEFAULT_COLS as usize)
        }

        /// Reports that writing to a PTY is not supported on this host.
        pub fn send(&self, _bytes: &[u8]) -> Result<()> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }

        /// Reports that typing into a PTY is not supported on this host.
        pub fn type_text(&self, _text: &str) -> Result<()> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }

        /// Reports that key presses are not supported on this host.
        pub fn press(&self, _key: &str) -> Result<()> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }

        /// Reports that resizing a PTY is not supported on this host.
        pub fn resize(&mut self, _rows: u16, _cols: u16) -> Result<()> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }

        /// Reports that waiting on a PTY is not supported on this host.
        pub fn wait_until(
            &self,
            _timeout_ms: u64,
            _pred: impl Fn(&ScreenState) -> bool,
        ) -> Result<ScreenState> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }

        /// Reports that waiting for screen text is not supported on this host.
        pub fn wait_for_text(&self, _text: &str, _timeout_ms: u64) -> Result<ScreenState> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }

        /// Reports that waiting for raw output is not supported on this host.
        pub fn wait_for_raw(&self, _text: &str, _timeout_ms: u64) -> Result<()> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }

        /// Reports that waiting for process exit is not supported on this host.
        pub fn wait_for_exit(&self, _timeout_ms: u64) -> Result<i32> {
            Err(TestError::Unsupported(UNSUPPORTED_MESSAGE.to_string()))
        }

        /// Indicates that no child process exists on this host.
        pub fn is_exited(&self) -> bool {
            true
        }

        /// Returns no exit status because no child process exists.
        pub fn exit_status(&self) -> Option<i32> {
            None
        }

        /// Returns the stable invalid descriptor sentinel for this unsupported backend.
        pub fn master_fd(&self) -> i32 {
            -1
        }

        /// Does nothing because no child process exists.
        pub fn kill(&self) {}
    }

    /// Reads one chunk from a regular file on non-Unix hosts.
    pub fn read_chunk(master: &mut std::fs::File) -> std::io::Result<Vec<u8>> {
        let mut buf = [0u8; 8192];
        let n = master.read(&mut buf)?;
        Ok(buf[..n].to_vec())
...
#[cfg(unix)]
pub use unix::{read_chunk, PtySession};
#[cfg(not(unix))]
pub use unsupported::{read_chunk, PtySession};
...
mod tests {
    ...
    fn test_read_chunk_from_file() {
        use std::io::Write;
        let path = std::env::temp_dir().join("rusty_testkit_chunk.tmp");
        let mut f = std::fs::File::create(&path).expect("create temporary chunk file");
        f.write_all(b"test chunk").unwrap();
        drop(f);

        let mut f_read = std::fs::File::open(&path).expect("open temporary chunk file");
        let chunk = read_chunk(&mut f_read).expect("read temporary chunk file");
        assert_eq!(chunk, b"test chunk");
        std::fs::remove_file(path).expect("remove temporary chunk file");
    }
}
